...
\begin{definition}
    ...
    A \emph{synthetic spectrum} is an additive sheaf 
    \[X\: \Sp^{\mathrm{fp}, \mathrm{op}}\to \Sp.\] 
    The category of synthetic spectra will be denoted 
    \[\SynE := \mathrm{Sh}_\Sigma(\Sp\fp;\Sp).\]
\end{definition}
...
\[E_* \: \Sp\to \ComodE.\index{Homology theory!Adapted}\] 
...
By \cref{ch1:lm:torsion-comodules-generated-by-compacts} from \hyperref[ch1]{Paper I} we know that the category of $I_n$-power torsion $E_*E$-comodules $\Comodt$ is compactly generated, with an explicit set of compact generators given by 
...
\begin{proof}
    ...
    \[\SW(\LSyn_{E, \geq 0}^\omega)\simeq \LSynE^\omega. \qedhere\]
\end{proof}
...
\begin{proof}
    ...
    \begin{tikzcd}[sep = scriptsize]
        \T\geqz \arrow[r, hook] & \LSyn_{E, \geq 0}^\omega,
    \end{tikzcd}
    which gives a fully faithful inclusion
    \begin{center}
    \begin{tikzcd}[sep = scriptsize]
        \T = \SW(\T\geqz) \arrow[r, hook] & \SW(\LSyn_{E, \geq 0}^\omega)\simeq \LSynE^\omega
    \end{tikzcd}
    \end{center}
...
We now claim that the unique lift $\MSynE$ of the compactly generated localizing subcategory $\Comodt$ is a compactly generated localizing subcategory. In the following result we let the stable lift of $\Thick(\TorsE)$ be denoted by $\T$. 
...
where $\TorsE$ denotes the collection of compact generators for the category $\Comodt$, see \cref{ch1:lm:torsion-comodules-generated-by-compacts}. 
...
The computation of the generic fibre of $\MSynE$ is very similar to \cref{ch3:add:thm:monochromatic-synthetic-special-fiber}, but we include it for completion. 
...
It is now natural to attempt to prove \cref{ch1:conj:monochromatic-monoidally-algebraic}---which states that at large enough primes the equivalence of $k$-categories 
\[h_k\M\np \simeq h_k\Fr\np\Int\] 
...
\end{corollary}
\begin{proof}
    The symmetric monoidal equivalence follows directly from \cref{ch3:add:cor:hypercomplete-at-large-primes}. The $t$-exactness also follows from this equivalence, as $\Der(E_*E\Int)$ is a $\pi$-stable localizing subcategory by \cite[3.7(2)]{barthel-heard-valenzuela_2020}, hence also compatible with the $t$-structure. 
\end{proof}

\begin{remark}
    It is possible to interpret $\Stable(\Comodt)$ as a category of sheaves itself, and run the same argument as for $\StableE$. This gives an even stronger claim, as $\Comodt$ has finite cohomological dimension $n^2+n$ whenever $p-1\nmid n$, as we proved in \cref{ch1:lm:cohomological-dimension-torsion-comodules}. This makes the situation very reminiscent of \cite[4.7]{barthel-heard_2018}, where Barthel--Heard prove an analogous statement for the Hopf algebroid $(E_*, K_*E)$. \index{Hopf algebroid}
\end{remark}
...
\end{corollary}
\begin{proof}
    Recall that we have a fully faithful $t$-exact inclusion 
    \begin{center}
    \begin{tikzcd}[sep = scriptsize]
        \MSynE \arrow[r, hook] &\LSynE.
    \end{tikzcd}
    \end{center}
    This functor has a right adjoint 
    \[M_n^\Syn \: \LSynE \to \MSynE.\] 
    As $M_n^\Syn$ is right adjoint to a $t$-exact functor, it is both left $t$-exact and limit-preserving. Hence, the claim follows from \cref{ch3:add:lm:LSynE-complete}.
\end{proof}
