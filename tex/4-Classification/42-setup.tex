...
Using this we can increase the strength of \cref{ch4:prop:induced-t-structure-on-stable-localizing} by also incorporating compatibility with filtered colimits. Recall that we use the name $t$-stable category for a stable category with a right complete $t$-structure compatible with filtered colimits. 
...
% \begin{lemma}
%     \label{ch4:lm:t-stable-iff-t-exact}
...
% We now prove our first results towards the wanted correspondences in \cref{ch4:thm:A} and \cref{ch4:thm:B}. 

% \begin{lemma}
%     \label{ch4:lm:stable-localizing-then-prestable-localizing}
...
% \begin{lemma}
%     \label{ch4:lm:t-stable-then-kernel-of-t-exact}
...
% \end{lemma}
% \begin{proof}
%     By \cref{ch4:lm:stable-localizing-then-prestable-localizing} $\L\geqz$ is a prestable localizing subcategory of $\C\geqz$. Hence it is determined as the kernel of a left exact localization $L\geqz\:\C\geqz\to\D\geqz$ by \cref{ch4:prop:Lurie-prestable-localizing-left-exact-functor}. In particular $L\geqz$ preserves colimits, hence we get by \cite[C.3.1.1]{lurie_SAG} a right $t$-exact functor $\Sp(L\geqz)\:\Sp(\C\geqz)\to \Sp(\D\geqz)$, which is also left $t$-exact by \cite[C.3.2.1]{lurie_SAG}. As $\Sp(-)$ preserves small limits by \cite[3.2.5]{lurie_SAG} we know that the kernel of the functor is preserved, i.e. that $\Sp(\L\geqz)$ is the kernel of $\Sp(L\geqz)$. By \cref{ch4:cor:t-stable-implies-equiv-to-sp} and \cref{ch4:lm:right-complete-then-equiv-to-sp} this implies that $\L$ is the kernel of a $t$-exact functor $L\:\C\to \D$, where we have denoted $\Sp(\L\geqz)=L$ and $\Sp(\D\geqz)=\D$. 
% \end{proof}
...
%     \label{ch4:lm:t-stable-iff-t-exact-localization}
...
% \begin{remark}
%     \label{ch4:rm:we-dont-need-right-exactness}
...
% We can now return to the study of $t$-exactness for the related inclusion and localization functors. We will make more precise the comment we made in \cref{ch4:rm:we-dont-need-right-exactness}, about left exactness usually being enough. 
...
% \begin{corollary}
%     \label{ch4:cor:inclusion-of-connected-localizing-left-exact}
...
% \end{corollary}
% \begin{proof}
%     The inclusion is exact and $t$-exact, hence this follows from \cref{ch4:lm:exact-functor-induces-left-exact-functor}, as the induced $t$-structure is right complete by \cref{ch4:prop:induced-t-structure-on-stable-localizing}.  
% \end{proof}

% \begin{construction}
%     Let $L\:\C\to\D$ be a localization of $t$-stable categories. This determines a localizing subcategory $\L$, and the fully faithful inclusion $\L\to \C$ is exact and preserves filtered colimits. Assuming that $\L$ is $t$-stable means that the inclusion is in addition $t$-exact. By \cref{ch4:lm:stable-localizing-then-prestable-localizing} we know that $\L\geqz$ is a prestable localizing subcategory of $\C\geqz$, and by \cref{ch4:cor:inclusion-of-connected-localizing-left-exact} the fully faithful inclusion $i\geqz\:\L\geqz \to \C\geqz$ is left exact. Since these are presentable categories, this has a right adjoint. In particular this means that $i\geqz$ i a left adjoint, hence is also right exact. 
...
% \begin{proposition}
%     \label{ch4:prop:stabilizing-localizing-is-localizing}
