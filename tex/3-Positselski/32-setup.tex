...
We denote the category of commutative algebras in $\C$ by $\CAlg(\C)$. These are the coherently commutative ring objects in $\C$. By \cite[2.4.2.7]{Lurie_HA} there is a symmetric monoidal structure on $\C\op$, and we define the category of $\E_\infty$-coalgebras in $\C$ to be the category $\cCAlg(\C):= \CAlg(\C\op)\op$. We will from now on omit the prefix $\E_\infty$ and refer to objects in $\cCAlg(\C)$ as \emph{commutative coalgebras}\index{Coalgebra!Cocommutative}, or simply just \emph{coalgebras}\index{Coalgebra}. Classical coalgebras will be referred to as \emph{discrete}\index{Coalgebra!Discrete} in order to avoid confusion. 
...
\[U_M\: \LMod_M(\D)\to \D.\] 
By \cite[4.2.4.8]{Lurie_HA} this functor admits a left adjoint $F_M\: \D\to \LMod_M(\D)$ given by $X\longmapsto MX$. We call this the free module functor. The adjunction $F_M\dashv U_M$ is called the \emph{free-forgetful} adjunction of $M$. 
