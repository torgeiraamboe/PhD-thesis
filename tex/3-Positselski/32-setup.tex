...
The goal of this section is to introduce comodules and contramodules over an $\E_\infty$-coalgebra in some $\infty$-category $\C$. In order to do this we first review some basic facts about coalgebras, monads and comonads. 
...
We denote the category of commutative algebras in $\C$ by $\CAlg(\C)$. These are the coherently commutative ring objects in $\C$. By \cite[2.4.2.7]{Lurie_HA} there is a symmetric monoidal structure on $\C\op$, and we define the category of $\E_\infty$-coalgebras in $\C$ to be the category $\cCAlg(\C):= \CAlg(\C\op)\op$. We will from now on omit the prefix $\E_\infty$ and refer to objects in $\cCAlg(\C)$ as \emph{commutative coalgebras}\index{Cocommutative coalgebra}, or simply just \emph{coalgebras}. Classical coalgebras will be referred to as \emph{discrete} in order to avoid confusion. 

\begin{proposition}
    The following properties hold for the category $\cCAlg(\C)$. 
    \begin{enumerate}
        \item The forgetful functor $U\:\cCAlg(\C)\to \C$ is conservative and creates colimits. \label{ch3:prop:properties-coalgebras:item1}
        \item The categorical product of two coalgebras $C, D$ is given by the tensor product of their underlying objects $C\otimes D$. \label{ch3:prop:properties-coalgebras:item2}
        \item The category $\cCAlg(\C)$ is presentably symmetric monoidal when equipped with the cartesian monoidal structure. In particular, this means that the forgetful functor $U$ is symmetric monoidal. \label{ch3:prop:properties-coalgebras:item3}
        \item The forgetful functor $U$ has a lax-monoidal right adjoint $cf\:\C\to\cCAlg(\C)$. The image of an object $X\in \C$ is called the cofree coalgebra on $X$. \label{ch3:prop:properties-coalgebras:item4}
    \end{enumerate}
\end{proposition}
\begin{proof}
...
% Old proof 
% \cref{ch3:prop:properties-coalgebras:item1} follows from \cite[3.2.2.5, 3.2.2.6]{Lurie_HA} while \cref{ch3:prop:properties-coalgebras:item2} follows from \cite[3.2.4.7]{Lurie_HA}. \cref{ch3:prop:properties-coalgebras:item3} is \cite[3.1.4]{lurie_2018_ELL1}, while \cref{ch3:prop:properties-coalgebras:item4} follows from \cref{ch3:prop:properties-coalgebras:item1} and \cref{ch3:prop:properties-coalgebras:item3} via the adjoint functor theorem \cite[5.5.2.9]{lurie_09}. 
...
\begin{definition}
    \index{Monad}
    \index{Comonad}
    A monad $M$ on $\D$ is an $\E_1$-algebra in $\Fun(\D, \D)$, and a comonad $C$ is an $\E_1$-coalgebra in $\Fun(\D, \D)$. 
\end{definition}
...
Given a monad $M$ on $\D$ we have a forgetful functor $U_M\: \LMod_M(\D)\to \D$. By \cite[4.2.4.8]{Lurie_HA} this functor admits a left adjoint $F_M\: \D\to \LMod_M(\D)$ given by $X\longmapsto MX$. We call this the free module functor. The adjunction $F_M\dashv U_M$ is called the \emph{free-forgetful} adjunction of $M$. 
...
\begin{definition}
    \index{Cofree comodule}
    \index{Comonadic adjunction}
...
\begin{example}
    \label{ch3:ex:algebra-module-monad}
...
\begin{example}
    \label{ch3:ex:coalgebra-comonad}
...
\begin{definition}
    \index{Contramodule}
    Let $C\in \cCAlg(\C)$ be a cocommutative coalgebra. A \emph{contramodule} over $C$ is a module over the internal hom-monad $\iHom_\C(C,-)\:\C\to \C$. The category of contramodules over $C$ in $\C$ is the corresponding Eilenberg--Moore category, which will be denoted $\ContraC(\C)$. 
\end{definition}
...
\begin{remark}
    \label{ch3:rm:internal-adjunction}
...
\begin{notation}
    We denote the mapping space in $\Comod_C$ by $\Hom_C$ and the mapping space in $\Contra_C$ by $\Hom^C$. Similarly, the forgetful functors will be denoted $U_C\:\Comod_C\to \C$ and $U^C\: \Contra_C\to \C$ respectively, while the cofree and free adjoints will be denoted $C\otimes (-)\:\C\to \Comod_C$ and $\iHom(C, -)\: \C\to \Contra_C$, hoping that it is clear from context whether we use them as above or as endofunctors on $\C$.
\end{notation}

The following proposition is standard for monads and comonads, see for example \cite[5.7]{riehl-verity_2015}. 

\begin{proposition}
    Let $C$ be a cocommutative coalgebra in $\C$. The forgetful functor $U_C\:\Comod_C\to \C$ creates colimits, while the forgetful functor $U^C\:\Contra_C\to \C$ creates limits. 
\end{proposition}
...
Let $F:\C\rightleftarrows \D:G$ pair of adjoint functors between symmetric monoidal categories, such that the left adjoint $F$ is symmetric monoidal. This means that the right adjoint $G$ is lax-monoidal, and does in particular preserve algebra objects. There is for any two objects $X\in \C$ and $Y\in \D$, a natural map
\[F(G(Y)\otimes_\C X)\overset{\simeq}\to FG(Y)\otimes_\D F(X) \to Y\otimes_\D F(X)\]
where the last map is given by the adjunction counit. By the adjunction property, there is an adjoint map 
\[G(Y)\otimes_\C X \to G(Y\otimes_\D F(X)).\]

\begin{definition}
    \index{Monadic projection formula}
...
\begin{theorem}[{\cite[5.29]{mathew-naumann-noel_2017}}]
    ...
    \end{enumerate}
    then the adjunction is monadic, and there is an equivalence of monads 
    \[GF \simeq G(\1_\D)\otimes_\C(-).\]
    In particular, there is a symmetric monoidal equivalence $\D\simeq \Mod_{G(\1_\D)}(\C).$
\end{theorem}

\begin{remark}
    Note that this result is stated only for stable $\infty$-categories in \cite{mathew-naumann-noel_2017}, but also holds unstably by a combination of Lurie's $\infty$-categorical Barr--Beck theorem, \cref{ch3:thm:Lurie-BB}, together with the fact that the monadic projection formula applied to the unit gives an equivalence of monads by \cite[3.6]{elmanto-kolderup_2020}. 
\end{remark}
...
\[G(F(X)\otimes_\D Y)\overset{\simeq}\to GF(X)\otimes_\C G(Y) \to X\otimes_\C G(Y)\]
where the last map is given by the adjunction unit. By the adjunction property, there is an adjoint map 
\[F(X)\otimes_\D Y \to F(X\otimes_\C G(Y)).\]

\begin{definition}
    \index{Comonadic projection formula}
...
\begin{theorem}
    \label{ch3:thm:dual-monoidal-BB}
...
\begin{remark}
    Before the proof we just remark why the above statement makes sense. The unit $\1_\C$ in a presentably symmetric monoidal $\infty$-category $\C$ is both an $\E_\infty$-algebra and an $\E_\infty$-coalgebra. In the above adjunction we have that the right adjoint $G$ is symmetric monoidal, hence its left adjoint $F$ is op-lax monoidal. In particular, it sends coalgebras to coalgebras, meaning that $F(\1_\C)$ is an $\E_\infty$-coalgebra in $\D$. By \cref{ch3:ex:coalgebra-comonad} tensoring with $F(\1_\C)$ is a comonad, not a monad, as for \cref{ch3:thm:monoidal-BB}. 
\end{remark}
...
\begin{proof}
    ...
    \begin{center}
        ...
        \end{tikzcd}
    \end{center}
    By applying the projection formula to the unit $\1_\C$ we get that $\phi$ is a natural equivalence, which means that the adjunction $(\phi_*, \phi^*)$ is an adjoint equivalence. By \cref{ch3:ex:coalgebra-comonad} the Eilenberg--Moore category of the comonad $F(\1_\C)\otimes_\D (-)$ is equivalent to the category of comodules over the $\E_\infty$-coalgebra $F(\1_\C)$, finishing the proof. 
\end{proof}

\begin{remark}
    \label{ch3:rm:monoidal-structure-comodules}
    We want to specify when the above equivalence is an equivalence of symmetric monoidal categories. We could hope for the existence of a symmetric monoidal structure on $\Comod_C$ for a cocommutative coalgebra $C\in \C$. For the category $\Mod_R(\C)$ of modules over a commutative algebra $R\in \C$ this is done by Lurie's relative tensor product, see \cite[Section 4.5.2]{Lurie_HA}. But, for such a relative monoidal product to exist on $\Comod_C$ one needs the tensor product in $\C$ to commute with cosifted limits, which is rarely the case. But, as we will see in \cref{ch3:ssec:idempotent-coalgebras} we sometimes get a monoidal structure, and when this is the case, the equivalence in \cref{ch3:thm:dual-monoidal-BB} is symmetric monoidal. 
\end{remark}
