...
\begin{proposition}
    If $p$ is a prime and $n$ a positive natural number, then the categories of comodules over the Hopf algebroids associated to the spectra $E\np$, $E_p(n)$ and $A = E\np^{h\F_p^\times}$, are all equivalent as symmetric monoidal categories: 
...
\begin{example}
    ...
    As we will use this functor a lot, we hammer in the fact that the above Morava $E$-theories are of Adams type. This means that the $E$-homology functor lands naturally in the symmetric monoidal Grothendieck abelian category $\ComodE$. We will mostly focus on the homology theory
    \[E_*\: \Sp\np \to \ComodE,\]
