...
\begin{definition}
    ...
    A spectrum $X$ is called {\defn $n$-monochromatic} if it is $E_n$-local and $E_{n-1}$-acyclic. The full subcategory of $n$-monochromatic spectra will be denoted {\defn $\M\np$} and referred to as the height $n$ monochromatic category.
\end{definition}
...
\begin{definition}
    Let $X\in \sp\np$. The fiber of the localization $X\longrightarrow L_{n-1}X$, which we denote $M_n X$ is called the $n$'th {\defn monochromatic layer} of $X$. 
\end{definition}
...
\begin{remark}
    ...
    gives an equivalence $X\simeq M_n X$. The fully faithful inclusion $\M\np\longrightarrow \sp\np$ has a right adjoint, given by $X\longmapsto M_nX$, which we call the {\defn monochromatization}. 
\end{remark}
...
\begin{definition}
    A compact $p$-local spectrum $X$ is said to be of {\defn type $n$} if $K_p(n)_* X\not\cong 0$ and $K_p(m)_*X\cong 0$ for all $m<n$. 
\end{definition}
