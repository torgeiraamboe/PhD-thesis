...
\begin{remark}
    The first two axioms makes $H$ a homological functor, the third makes $H$ into a locally graded functor, i.e., a functor that preserves the local grading, and the last makes it a conservative functor. 
\end{remark}

\begin{example}
    Let $R$ be a ring spectrum. In this situation the functor $\pi_*\: \Mod_R\longrightarrow \Mod_{R_*}$ defined as $\pi_* M = [\S, M]_*$ is a conservative homology theory. 
\end{example}

\begin{example}
    If $R$ is a ring spectrum, then the functor 
    \[R_*(-)\: \Sp \longrightarrow \Mod_R,\] 
...
\begin{example}
    We again return to the present paper's two guiding examples, being 
    \[\pi_*\: \Mod_R\longrightarrow \Mod_{R_*} \quad\text{ and }\quad R_*\: \Sp_R\longrightarrow \Comod_{R_*R},\] 
    where $R$ is an Adams type ring spectrum. Both functors are conservative adapted homology theories, with faithful lifts provided by Brown representability, see \cite[8.2]{patchkoria-pstragowski_2021} and \cite[8.13]{patchkoria-pstragowski_2021} respectively. 
\end{example}
...
\begin{remark}
    Recall that we are really interested in the category $\SpKpn$ of $\Kpn$-local spectra.\index{Spectra!$K_p(n)$-local} The spectrum $\Kpn$ is a field object in $\Sp$, and its homotopy groups $\pi_* \Kpn$ are graded fields. Hence the homology theory 
    \[\Kpn_*\: \SpKpn\longrightarrow \Mod_{\Kpn_*}\] 
    is too simple to exhibit the algebraicity properties that we want. As $\Kpn$ is Adams type $\Kpn_*(-)$ factors through $\Comod_{K_*K}$, but this category is very complicated. In particular, it does not have a finite cohomological dimension, a feature we will need later. This can be seen via the following argument of \cite{barthel-pstragowski_2021}. Having a finite cohomological dimension would imply that the $\Kpn$-Adams spectral sequence has a horizontal vanishing line at a finite page. The groups in this spectral sequence are all torsion, hence this would imply that, for example, the homotopy groups of the $\Kpn$-local sphere is a finite filtration of torsion groups. In particular there could be no rational homotopy groups. But, by \cite{barthel_schlank_stapleton_weinstein_2024} the rational homotopy groups of the $\Kpn$-local sphere are highly non-trivial, meaning that the original assumption that $\Comod_{K_*K}$ has finite cohomological dimension must be wrong. 
...
For certain Adams type ring spectra $R$, we get decompositions of the category $\Comod_{R_*R}$ into periodic families of subcategories. Such decompositions allow for the construction of partial inverses to the associated homology theories. 
...
\begin{construction}
    ...
    where $\Comod_{R_*R, \phi}^{inj}$ denotes the full subcategory spanned by injective comodules concentrated in degrees divisible by $\phi$. 
...
\begin{definition}[\cite{franke_96}]
    ...
    \index{Splitting}
    Let $\A$ be an abelian category with a local grading $[1]$. A \emph{splitting} of $\A$ of order $q+1$ is a collection of Serre subcategories\index{Serre subcategory} $\A_\phi \subseteq \A$ indexed by $\phi \in \Z/(q+1)$ satisfying
    \begin{enumerate}
...
\begin{definition}
    ...
    \index{Homology theory!$k$-exotic}
     Let $H\: \C\longrightarrow \A$ be a homology theory. We say $H$ is \emph{$k$-exotic} if $H$ is adapted, conservative, $\A$ has finite cohomological dimension $d$ and a splitting of order $q+1$ such that $k=q+1-d>0$. 
\end{definition}
...
\begin{theorem}[{\cite[7.56]{patchkoria-pstragowski_2021}}]
    ...
    \index{Franke's algebraicity theorem}
