...
\begin{remark}
    ...
    There is, however, a version of $E_*$-homology on $\SpK$, defined by sending a $K(n)$-local spectrum $X$ to 
    \[E_*^\vee (X) := \pi_*L_{K_p(n)}(E\otimes X).\]
...
\begin{example}[{\cite[8.7]{patchkoria-pstragowski_2021}}]
    ...
    Let $p$ be a prime, $n$ be a non-negative integer, and $E$ a height $n$ Morava $E$-theory concentrated in degrees divisible by $2p-2$, for example Johnson-Wilson theory $E(n)$. If $k=2p-2-n>0$, then the functor 
